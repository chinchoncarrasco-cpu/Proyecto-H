-- La vista financiera redistribuye los pagos de alojamiento por orden dentro
-- de cada reserva. Su límite debe validarse con el total ajustado de la reserva;
-- los demás conceptos mantienen el límite ajustado de su cargo individual.
...
begin
  ...
  select c.reserva_id, c.estado, c.tipo_cargo, ec.monto_ajustado
    into v_reserva_cargo, v_estado_cargo, v_tipo_cargo, v_monto_cargo
  ...
  if v_tipo_pago = 'pago' then
    if v_tipo_cargo = 'alojamiento' then
      select coalesce(sum(ec.monto_ajustado), 0)
        into v_monto_alojamiento
      from public.vista_estado_cargos ec
      where ec.reserva_id = v_reserva_cargo
      ...
      where c.reserva_id = v_reserva_cargo
        and c.tipo_cargo = 'alojamiento'
        and c.estado = 'activo'
        and p.estado = 'confirmado'
        and pa.id <> new.id;
      if v_neto_cargo + new.monto_aplicado > v_monto_alojamiento then
        raise exception 'La aplicación supera el saldo de alojamiento disponible de la reserva';
      end if;
    else
      select coalesce(sum(
        case when p.tipo_movimiento = 'pago' then pa.monto_aplicado
