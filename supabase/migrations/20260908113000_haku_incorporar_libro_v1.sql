-- Haku Libro <-> Proyecto H
-- Incorpora una seleccion ya revisada como una sola transaccion e impide
-- repetir la misma operacion si el cliente reintenta la llamada RPC.

create table if not exists private.haiku_libro_incorporaciones (
...
revoke all on table private.haiku_libro_incorporaciones from public, anon, authenticated;

create or replace function private.haiku_libro_agregar_estadia_v1(
  p_reserva_id uuid,
  p_estadia jsonb
)
returns jsonb
...
declare
  ...
  v_fecha date;
begin
  if jsonb_typeof(coalesce(p_estadia, '{}'::jsonb)) <> 'object' then
    raise exception 'La estadia debe ser un objeto';
  end if;

  begin
    v_cabana := (p_estadia ->> 'cabana_numero')::smallint;
    v_ingreso := (p_estadia -> 'datos' ->> 'fecha_ingreso')::date;
    v_salida := (p_estadia -> 'datos' ->> 'fecha_salida')::date;
    v_adultos := (p_estadia -> 'datos' ->> 'adultos')::smallint;
    v_ninos := coalesce((p_estadia -> 'datos' ->> 'ninos')::smallint, 0);
    v_mascotas := coalesce((p_estadia -> 'datos' ->> 'mascotas')::smallint, 0);
  exception when others then
    raise exception 'La estadia contiene cabana, fechas u ocupacion invalidas';
  end;

  v_tipo := lower(btrim(coalesce(p_estadia -> 'datos' ->> 'tipo_estadia', '')));
  if v_cabana is null or v_cabana <= 0 or v_ingreso is null or v_salida is null then
    raise exception 'La estadia requiere cabana y fechas';
  end if;
  if v_adultos is null or v_adultos < 1 or v_ninos < 0 or v_mascotas < 0 then
    raise exception 'La ocupacion de la estadia no es valida';
  end if;
  if v_tipo not in ('alojamiento', 'fullday') then
    raise exception 'Tipo de estadia invalido';
  end if;
  if (v_tipo = 'alojamiento' and v_salida <= v_ingreso)
     or (v_tipo = 'fullday' and v_salida <> v_ingreso) then
    raise exception 'Las fechas no corresponden al tipo de estadia';
  end if;

  if not exists (
    select 1 from public.reservas r
    where r.id = p_reserva_id and r.estado_reserva <> 'cancelada'
  ) then
    raise exception 'La reserva destino no existe o esta cancelada';
  end if;

  select c.id, c.precio_base
    into v_cabana_id, v_precio_base
  from public.cabanas c
  where c.numero = v_cabana and c.activa = true;
  if not found then
    raise exception 'La CAB % no existe o esta inactiva', v_cabana;
  end if;

  select re.id into v_estadia_id
  from public.reserva_estadias re
  where re.reserva_id = p_reserva_id
    and re.cabana_id = v_cabana_id
    and re.fecha_ingreso = v_ingreso
    and re.fecha_salida = v_salida
    and re.tipo_estadia = v_tipo
  ...
  if v_estadia_id is not null then
    return jsonb_build_object(
      ...
      'omitida', true,
      'motivo', 'La estadia ya existe en la reserva'
    );
  end if;

  if not exists (
    select 1
    from public.haiku_cabanas_disponibles(v_ingreso, v_salida, v_tipo) d
    where d.numero = v_cabana
  ) then
    raise exception 'La CAB % ya no esta disponible para esas fechas', v_cabana;
  end if;
  if coalesce(v_precio_base, 0) <= 0 then
    raise exception 'La CAB % no tiene una tarifa base valida', v_cabana;
  end if;
  ...
  insert into public.reserva_estadias(
    ...
    p_reserva_id, v_cabana_id, v_ingreso, v_salida,
    v_adultos, v_ninos, v_mascotas, 'pendiente', v_tipo
  ...
  if v_tipo = 'alojamiento' then
    v_fecha := v_ingreso;
    while v_fecha < v_salida loop
      insert into public.estadia_noches(estadia_id, fecha, tarifa, origen_tarifa)
      values (v_estadia_id, v_fecha, v_precio_base, 'catalogo');
      v_fecha := v_fecha + 1;
    end loop;
  else
    insert into public.cargos(
      reserva_id, estadia_id, estadia_noche_id, tipo_cargo,
      concepto, monto, moneda, estado, creado_por
    ) values (
      p_reserva_id, v_estadia_id, null, 'alojamiento',
      'Full Day - ' || to_char(v_ingreso, 'DD-MM-YYYY'),
      v_precio_base, 'CLP', 'activo', auth.uid()
    );
  end if;
...
revoke all on function private.haiku_libro_agregar_estadia_v1(uuid, jsonb)
  ...
  loop
    ...
    where p.tipo_movimiento = 'pago'
      ...
      and (
        ...
        or (p.reserva_id = v_reserva_id and p.monto = v_monto and p.moneda = 'CLP')
      )
    ...
    if v_pago_existente is not null then
      v_omitidos := v_omitidos || jsonb_build_array(jsonb_build_object(
        ...
        'motivo', 'El pago ya existe o coincide con la misma reserva y monto'
      ));
      continue;
    end if;
